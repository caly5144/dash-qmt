...
def run_sync():
    """执行日线数据增量同步的主函数"""
    print(f"【独立任务】开始执行日线数据同步 {datetime.now()}")
    ...
    if not last_update_map:
        start_time_str = '20100101'
        print("【全量】数据库为空，下载起始日期: 20100101")
    else:
        # 增量模式：回推15天以防遗漏
    ...
    print(f"【完成】同步结束，新增 {total_inserted} 条，时间：{datetime.now()}")
